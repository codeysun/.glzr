...
<div class="flex flex-row gap-6 items-center">
  ...
  {#if weather}
    <div>
      ...
      {/if}
      <div class="relative inline-flex items-center justify-center group" style="padding-right: 6px;">
        <span
          class="flex items-center transform -translate-y-full opacity-0 transition-all duration-300 ease-in-out group-hover:translate-y-0 group-hover:opacity-100"
        >
          {Math.round(weather.celsiusTemp)} °C
        </span>
        <span
          class="absolute inset-0 inline-block items-center transition-all duration-300 ease-in-out group-hover:translate-y-full group-hover:opacity-0"
        >
  ...
  <div class="relative inline-flex items-center justify-center group" style="padding-right: 6px;">
    <span
      class="flex items-center transform -translate-y-full opacity-0 transition-all duration-300 ease-in-out group-hover:translate-y-0 group-hover:opacity-100"
    >
      <i class="ti ti-calendar-month" style="margin-right: 7px;"></i>
      {date?.formatted}
    </span>
    <span
      class="absolute inset-0 inline-block items-center transition-all duration-300 ease-in-out group-hover:translate-y-full group-hover:opacity-0"
    >
      <i class="ti ti-clock" style="margin-right: 7px;"></i>
      {time?.formatted}
    </span>
  </div>
</div>
